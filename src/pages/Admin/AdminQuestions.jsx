...
const AdminQuestions = () => {
    ...
    const [currentQuestion, setCurrentQuestion] = useState({
        ...
        options: [{ option_text: '', domain_id: '', points: 5 }]
    });
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            fetchQuestions();
            setCurrentQuestion({ question_text: '', language: i18n.language, options: [{ option_text: '', domain_id: '', points: 5 }] });
        } catch (error) {
    ...
    const handleEdit = (question) => {
        setCurrentQuestion({
            ...question,
            question_text: getLocalizedText(question.question_text),
            language: i18n.language,
            options: question.options.map(opt => ({
                ...opt,
                option_text: getLocalizedText(opt.option_text)
            }))
        });
        setIsEditing(true);
        setIsModalOpen(true);
    ...
    const handleGenerateAi = async () => {
        ...
        try {
            ...
            alert(t('admin.error.generateAI', 'Erreur lors de la génération IA'));
        } finally {
    ...
    return (
        <div className="admin-questions">
            <header className="admin-header">
                <div>
                    <h1>{t('admin.management.questions', 'Gestion des Questions')}</h1>
                    <p>{t('admin.management.questionsSubtitle', 'Configurez les questions du test d\'orientation et leurs points')}.</p>
                </div>
                <div className="header-actions">
                    <div className="ai-controls">
                        <input 
                            ...
                            title={t('admin.management.lessonCount', 'Nombre de questions à générer')}
                        />
                        <button 
                    ...
                    <button className="btn btn-primary" onClick={handleAddNew}>
                        <Plus size={20} /> {t('admin.management.addQuestion', 'Ajouter une Question')}
                    </button>
                </div>
            </header>
                            ...
                            <div className="options-summary">
                                {question.options.map((opt, idx) => (
                                    <span key={idx} className="option-badge">
                                        {getLocalizedText(opt.option_text)} ({getLocalizedText(opt.domain?.name) || opt.domain_id})
                                    </span>
                                ))}
                            </div>
                        </div>
                ...
                <div className="modal-overlay">
                    <div className="modal ai-preview-modal card">
                        <div className="modal-header">
                            <h2>{t('admin.management.aiPreview', 'Prévisualisation IA')}</h2>
                        ...
                        <div className="ai-questions-preview">
                            <p className="helper-text">{t('admin.management.aiPreviewNote', 'Vérifiez et modifiez les questions générées avant de les valider.')}</p>
                            {aiPreview.map((q, idx) => (
                                <div key={idx} className="preview-item card" style={{ padding: '24px', marginBottom: '24px', border: '1px solid #e2e8f0' }}>
                                    ...
                                    <div className="options-editor" style={{ marginTop: '20px' }}>
                                        <div className="options-header">
                                            <h4>{t('admin.management.responseOptions', 'Options de réponse')}</h4>
                                        </div>
                                        {q.options.map((opt, oIdx) => (
                                            <div key={oIdx} className="option-edit-row" style={{ gridTemplateColumns: '2fr 1fr 80px' }}>
                                                ...
                                                <select 
                                                    ...
                                                    required
                                                >
                                                    <option value="">{t('admin.domains', 'Domaine')}...</option>
                                                    {domains.map(d => <option key={d.id} value={d.id}>{getLocalizedText(d.name)}</option>)}
                                                </select>
                                                <input 
                                                    type="number" 
                                                    placeholder="Pts" 
                        ...
                        <div className="modal-footer">
                            <button className="btn btn-outline" onClick={() => setIsAiModalOpen(false)}>{t('common.cancel', 'Annuler')}</button>
                            <button 
                            ...
                            >
                                ...
                                {t('admin.management.validateAll', 'Valider et Ajouter tout')}
                            </button>
                        </div>
                    </div>
                </div>
            ...
            {isModalOpen && (
                <div className="modal-overlay">
                    <div className="modal question-modal card">
                        <h2>{isEditing ? t('common.edit', 'Modifier') : t('common.add', 'Ajouter')} {t('admin.questions', 'une question')}</h2>
                        <form onSubmit={handleSave}>
                            <div className="input-group">
                                <label>{t('admin.management.questionText', 'Texte de la question')}</label>
                                <input 
                                    type="text" 
                            ...
                            <div className="options-editor">
                                <div className="options-header">
                                    ...
                                    <button type="button" className="btn btn-outline btn-sm" onClick={handleAddOption}>
                                        <Plus size={16} /> {t('admin.management.addOption', 'Ajouter une option')}
                                    </button>
                                </div>
                                ...
                                {currentQuestion.options.map((option, index) => (
                                    <div key={index} className="option-edit-row animate-fade-in">
                                        ...
                                        <input 
                                            ...
                                            required
                                        />
                                        <button type="button" className="btn-remove" onClick={() => handleRemoveOption(index)} title={t('common.delete', 'Supprimer')}>
                                            <X size={18} />
                                        </button>
                                    </div>
                                ))}
